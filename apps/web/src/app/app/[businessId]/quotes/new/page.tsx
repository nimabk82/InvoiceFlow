...
export default function NewQuotePage() {
  ...
  const [depositDueRule, setDepositDueRule] = useState<DepositDueRule>("on_receipt");
  ...
  function addTax(itemIndex: number) {
    setItems((current) =>
      current.map((item, i) =>
        i === itemIndex
          ? { ...item, appliedTaxes: [...(item.appliedTaxes ?? []), { name: "", rate: "" }] }
          : item,
      ),
    );
  }
  function updateTax(itemIndex: number, taxIndex: number, patch: Partial<{ name: string; rate: string }>) {
    setItems((current) =>
      current.map((item, i) => {
  ...
  function removeTax(itemIndex: number, taxIndex: number) {
    setItems((current) =>
      current.map((item, i) =>
        i === itemIndex
          ? { ...item, appliedTaxes: (item.appliedTaxes ?? []).filter((_, ti) => ti !== taxIndex) }
          : item,
      ),
    );
  }
      ...
      <Card sx={{ p: 2, mb: 3 }}>
        ...
        {selectedClient ? (
          <div className="if-client-selector">
            ...
            <Button variant="outlined" onClick={() => setClientId("")}>Change</Button>
          </div>
        ) : (
          <Select
            id="client-select"
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, mt: 2 }}>
          <Input label="Quote #" value={number} onChange={(event) => setNumber(event.target.value)} placeholder="Q-001" />
        ...
        <div>
          <Card sx={{ overflow: "hidden" }}>
            ...
            {items.map((item, index) => (
              <div key={index} className="if-row-grid if-row" style={{ gridTemplateColumns: "minmax(200px,1fr) 90px 120px 110px 40px" }}>
                <div>
                  <Input
                    size="small"
                    label="Description"
                  ...
                  {(item.appliedTaxes ?? []).map((tax, taxIndex) => (
                    <Box key={taxIndex} sx={{ display: "flex", gap: 1, mt: 1 }}>
                      <Input size="small" label="Tax name" value={tax.name} onChange={(event) => updateTax(index, taxIndex, { name: event.target.value })} />
                      <Input size="small" label="%" value={tax.rate} onChange={(event) => updateTax(index, taxIndex, { rate: event.target.value })} sx={{ width: 80 }} />
                      <IconButton aria-label="Remove tax" onClick={() => removeTax(index, taxIndex)}><DeleteIcon /></IconButton>
                    </Box>
                  ))}
                  <Button variant="text" size="small" onClick={() => addTax(index)}>+ Tax</Button>
                </div>
                <Input size="small" label="Qty" value={item.quantity} onChange={(event) => updateItem(index, { quantity: event.target.value })} />
                <Input size="small" id={`items[${index}].rate`} label="Rate" value={item.rate} onChange={(event) => updateItem(index, { rate: event.target.value })} />
                <div className="if-right if-amount money">{totals?.lineTotals[index]?.toDecimalString() ?? "0.00"}</div>
                <IconButton aria-label="Remove item" onClick={() => removeItem(index)}><DeleteIcon /></IconButton>
              </div>
            ))}
              ...
              {totals.deposit && (
                <div className="if-depositbox">
                  <div className="if-dtop">
                    <span className="if-label">Deposit upon acceptance</span>
